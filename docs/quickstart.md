...
```bash
# ⓪ 同梱PDFを S3(MinIO) にアップロード（→ s3://biblio/raw/sample_book.pdf）
...
```bash
# 1) PDF を S3(MinIO) にアップロード（例: mybook.pdf → book_id = mybook）
uv run python -m workers.upload /path/to/mybook.pdf
#    （MinIO コンソール http://localhost:9001 からドラッグ&ドロップでも可。raw/ に置く）

# 2) 必須メタデータ（title / author）
echo '{"title": "書名", "author": "著者名"}' > books/mybook.meta.json

# 3) 抽出→チャンク→埋め込み（引数なしで S3 の raw/ と books/ 配下を一括処理）
uv run python -m workers.extract
uv run python -m workers.chunk
uv run python -m workers.embed
