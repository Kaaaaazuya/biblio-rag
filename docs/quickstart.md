...
# ① 抽出: S3(raw/) の PDF → S3(normalized/)
uv run python -m workers.extract

# ② チャンク: S3(normalized/) → S3(chunks/)
uv run python -m workers.chunk

# ③ 埋め込み + 格納: S3(chunks/) → pgvector
...
> `HYBRID_ENABLED=true` は pg_bigm 拡張入りの Docker イメージが必要（現在の `docker-compose.yml` では無効）。

4条件を一括比較（fixture クエリで相対評価）:
...
```bash
uv run pytest                 # 単体テスト（高速・インフラ不要。E2E は除外）
uv run pytest -m e2e          # パイプライン E2E（要: スタック起動。実データは汚さない）
uv run pytest -m webui        # WebUI E2E（要: スタック + 初回 `uv run playwright install chromium`）
```

E2E はインフラ未起動なら skip する。テスト全体の方針は [testing.md](testing.md)（設計の経緯は [ADR 0010](adr/0010-e2e-testing-strategy.md)）。
