...
| ストレージ | MinIO（S3互換・raw/normalized/chunks） | Amazon S3 |
...
| ベクトル DB | pgvector on Docker | Neon（サーバレス Postgres）+ pgvector |
| コスト | ¥0 | 従量課金（ベクトル DB は Neon 無料枠） |

次元を 1024 で揃えるとスキーマ（`VECTOR(1024)`）・インデックス定義を変えずモデルだけ差し替えられる。
ただし**意味空間はモデルごとに別物**なので、本番移行時は `chunks/*.jsonl` から再埋め込みする
（→ [ADR 0006](adr/0006-local-to-aws.md)）。ベクトル DB は Aurora ではなく Neon を採用する
（アイドル課金ゼロの制約を満たせるのが Neon のみのため。→ [ADR 0015](adr/0015-zero-cost-deployment.md)）。
...
| VectorStore | PgVectorStore（Docker） | PgVectorStore（Neon・同一実装） |
...
3. AWS リソース作成（SQS×3+DLQ×3、S3、Fargate タスク定義、Lambda×2、Secrets Manager）＋ Neon プロジェクト作成（ベクトル DB のみ非 AWS）。
4. 設定のみ切替（Embedder→BedrockEmbedder、SQS/S3 エンドポイント→本番、`DATABASE_URL`→Neon、環境変数で制御）。
5. `chunks/*.jsonl` から BedrockEmbedder で再埋め込み → Neon pgvector へ投入。
...
| DB 認証 | Secrets Manager | Lambda が Neon 接続文字列（`DATABASE_URL`）を取得 |
| ベクトル DB | pgvector | 開発(Docker)/本番(Neon)でエンジン・スキーマ共通。Aurora はアイドル課金ゼロを満たせず不採用（→ [ADR 0015](adr/0015-zero-cost-deployment.md)） |
