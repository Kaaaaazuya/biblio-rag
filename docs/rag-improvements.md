...
query
  ...
  ↓ [ADJACENT_CHUNK_ENABLED]  ヒットチャンクの前後（chunk_index ±window）を追加取得
  → list[dict]（空の場合 SCORE_THRESHOLD_ENABLED 時は LLM を呼ばず「該当情報なし」を返す）
...
[Step 5] HyDE — _retrieve 前段に仮説生成追加  ✅ 完了（main PR #3）
  ↓
[Step 6] Score Threshold — ベクトル検索直後にスコア閾値未満を除外、
         全除外時は LLM を呼ばず「該当情報なし」を返す  ✅ 完了（PR #32、→ ADR 0013）
  ↓
[Step 7] Adjacent Chunk Expansion — ヒットチャンクの前後（chunk_index ±window）
         を追加取得し文脈の連続性を補う  ✅ 完了（PR #32、→ ADR 0013）
```
