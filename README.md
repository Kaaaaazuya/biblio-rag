...
`books/raw/` に PDF を置き、`title`/`author` のサイドカー JSON を用意して直列実行する。
（`book_id` はファイル名 stem。`books/` は `.gitignore` 済みでコミットされない）

```bash
# 0) スタック起動 + モデル取得（上記「開発スタックの起動」を実施済みとする）

# 1) 書籍と必須メタデータを配置（例: your_book.pdf → book_id = your_book）
cp your_book.pdf books/raw/
echo '{"title": "書名", "author": "著者名"}' > books/your_book.meta.json

# 2) ① 抽出: books/raw/*.pdf → books/normalized/*.md
...
books/            # 書籍データ（.gitignore・コミット禁止）
  raw/            #   原本 PDF
  normalized/     #   ① 抽出済み Markdown（正本・ディスク/S3）
  chunks/         #   ② 分割済み JSONL（正本・ディスク/S3）
workers/
  extract/        # ① PyMuPDF 抽出
  chunk/          # ② チャンク（サイズ可変）
